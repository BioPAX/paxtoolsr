...
file1 <- system.file("extdata", "dna_replication.owl", package="paxtoolsr")
...
errorLog <- validate(system.file("extdata", "dna_replication.owl", package="paxtoolsr"), onlyErrors=TRUE) 
```
...
These results can be filtered using the **XML** package using XPath expressions; [examples of XPath expressions and syntax](http://www.w3schools.com/xpath/xpath_examples.asp). The examples here shows how to pull the name for the [[FIX]] where the ID exists and pathways in which it participates. 
...
```{r saveBiopaxFileFromPcQuery, message=FALSE}
...
```{r gseaExampleGenGeneSet, results='hide'}
# Generate a Gene Set 
